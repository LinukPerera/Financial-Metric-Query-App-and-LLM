...
import logging
import tempfile
from metric_dictionary import metric_mappings, suggested_queries, suggested_sectors
from DebugHeader import detect_multirow_sheet
...
def get_available_sectors(processor):
    """Get available sectors from the uploaded Excel file."""
    if not processor:
        return suggested_sectors
    
    try:
        # Get sectors from the primary sheet
        primary_sheet_name = detect_multirow_sheet(processor.wb)
        available_sectors = processor._get_all_sectors(primary_sheet_name)
        
        # Combine with suggested sectors and remove duplicates
        all_sectors = list(set(available_sectors + suggested_sectors))
        all_sectors.sort()
        return all_sectors
    except Exception as e:
        logging.warning(f"Could not load sectors from file: {e}")
        return suggested_sectors

def get_query_types(processor=None):
    sheet_names = processor.wb.sheetnames if processor else ["Upload Excel file first"]
    # Get unique metrics by converting to set and back to list
    unique_metrics = list(set(metric_mappings.values()))
    available_sectors = get_available_sectors(processor)
    ...
    return [
        {
            ...
            "parameters": [
                {"name": "metric", "type": "selectbox", "options": unique_metrics, "label": "Select Metric"},
        ...
        },
        {
            ...
            "parameters": [
                {"name": "metric1", "type": "selectbox", "options": unique_metrics, "label": "Select First Metric"},
                {"name": "metric2", "type": "selectbox", "options": unique_metrics, "label": "Select Second Metric"},
                {"name": "company", "type": "text_input", "label": "Enter Company Code (e.g., ALLI)"}
            ],
            "query_format": "{metric1} and {metric2} for {company}",
            "example": "P/E and Revenue 3M for EDEN"
        },
        {
        ...
        },
        {
            ...
            "parameters": [
                {"name": "metric", "type": "selectbox", "options": unique_metrics, "label": "Select Metric"},
                {"name": "sector", "type": "selectbox", "options": available_sectors, "label": "Select Sector"}
            ],
        ...
        {
            "template": "best a",
            "display": "Best Stock by Metric Value",
            "parameters": [
                {"name": "metric", "type": "selectbox", "options": unique_metrics, "label": "Select Metric"}
            ],
        ...
        {
            "template": "lowest a",
            "display": "Find Stock with lowest Lowest Metric Value",
            "parameters": [
                {"name": "metric", "type": "selectbox", "options": unique_metrics, "label": "Select Metric"}
            ],
        ...
        {
            "template": "highest a",
            "display": "Find Stock with Highest Metric Value",
            "parameters": [
                {"name": "metric", "type": "selectbox", "options": unique_metrics, "label": "Select Metric"}
            ],
        ...
        },
        {
            ...
            "parameters": [
                {"name": "stocks", "type": "text_input", "label": "Enter Stock Codes (comma-separated, e.g., ALLI, DIAL, COMB)"},
                {"name": "metric", "type": "selectbox", "options": unique_metrics, "label": "Select Metric"}
            ],
            "query_format": "compare stocks {stocks} by {metric}",
            "example": "compare stocks ALLI, DIAL, COMB by P/E"
        },
        {
            ...
            "example": "compare stocks ALLI, DIAL"
        },
        {
            ...
            "parameters": [
                ...
                {"name": "stock2", "type": "text_input", "label": "Enter Second Stock Code (e.g., PALM)", "key": "stock2"},
                {"name": "sector", "type": "selectbox", "options": available_sectors, "label": "Select Sector"},
                {"name": "metric", "type": "selectbox", "options": unique_metrics, "label": "Select Metric"}
            ],
            "query_format": "{stock1} vs {stock2} vs sector {sector} by {metric}",
            "example": "ALLI vs PALM vs sector BANKS by P/E"
        },
        {
        ...
        },
        {
            ...
            "parameters": [
                {"name": "sector1", "type": "selectbox", "options": available_sectors, "label": "Select First Sector"},
                {"name": "sector2", "type": "selectbox", "options": available_sectors, "label": "Select Second Sector"},
                {"name": "metric", "type": "selectbox", "options": unique_metrics, "label": "Select Metric"}
            ],
        ...
        },
        {
            ...
            "parameters": [
                {"name": "display_metric", "type": "selectbox", "options": unique_metrics, "label": "Select Display Metric"},
                {"name": "filter_metric", "type": "selectbox", "options": unique_metrics, "label": "Select Filter Metric"},
...
"""
        ...
        with st.container():
            ...
            user_query = st.text_input(
                ...
                placeholder="e.g., P/E for ALLI or ALLI vs JKH vs sector BANKS by P/E",
                key="custom_query"
            )
